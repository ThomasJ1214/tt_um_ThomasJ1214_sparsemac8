...
calculation one instruction at a time. The result comes back on `uo[7:0]`.

Reading the answer needs one word of warning. The `uo[7:0]` pins drive the board's 7-segment display one
segment per bit, so a design like this one, which puts a plain number on those pins, lights an arbitrary
pattern of segments rather than showing a digit. Read the value as a binary number from the output pins in the
Commander app, or off the output LEDs, and convert. The segment pattern is not meant to be read as a number.
...
RP2040 for the control pins and for stepping the clock, and the output pins for reading the result back.
